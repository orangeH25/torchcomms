...
TorchWorkHCCL::WorkStatus TorchWorkHCCLQueue::garbageCollect(
    bool isMainThread) {
  std::lock_guard<std::recursive_mutex> lock(work_queues_mutex_);

  TorchWorkHCCL::WorkStatus last_status = TorchWorkHCCL::WorkStatus::COMPLETED;
  ...
  while (it != stream_work_queues_.end()) {
    ...
    while (!work_queue.empty()) {
      ...
      TorchWorkHCCL::WorkStatus status = work->checkStatus();
      last_status = status;

      if (status == TorchWorkHCCL::WorkStatus::COMPLETED) {
        ...
        // Continue to the next element in the queue
          ...
          status == TorchWorkHCCL::WorkStatus::TIMEDOUT ||
          status == TorchWorkHCCL::WorkStatus::ERROR) {
...
TorchWorkHCCL::WorkStatus TorchWorkHCCLQueue::finalize() {
  ...
  // empty
  TorchWorkHCCL::WorkStatus status = TorchWorkHCCL::WorkStatus::COMPLETED;
  while (!stream_work_queues_.empty()) {
    status = garbageCollect(true);
    if (status == TorchWorkHCCL::WorkStatus::ERROR ||
        status == TorchWorkHCCL::WorkStatus::TIMEDOUT ||
        status == TorchWorkHCCL::WorkStatus::COMPLETED) {
      break;
    }
  }
...
void TorchWorkHCCLQueue::enqueueWork(
